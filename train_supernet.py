...
def _compute(self, pred, tea_pred, label=None, *args):
    ...
    pred = paddle.slice(
        ...
        # pred, axes=[len(pred.shape) - 1], starts=[0], ends=[1])
...
def main(cfg):
    ...
    if train_flag:
        ...
        #model.evaluate(val_set, batch_size=cfg.batch_size, num_workers=0, eval_sample_num=10)
    else:
        model.evaluate(val_set, batch_size=cfg.batch_size, num_workers=0, eval_sample_num=0)
